...
#### 18. Top 5 Customers by Sales (per State)
```sql
WITH cte AS (
    SELECT 
        c.customer_id,
...
#### 19. Top 5 Customers by Orders (per State)
```sql
WITH cte AS (
    SELECT 
        c.customer_id,
...
#### 20. Revenue by Shipping Provider
...
#### 21. Top 10 Products by Growth (2023 → 2024)
```sql
WITH cte AS (
    SELECT
        p.product_id,
...
#### 22. Stored Procedure: Update Inventory after Sale
